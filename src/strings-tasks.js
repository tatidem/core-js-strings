...
function getCardId(value) {
  const cardValues = 'A23456789XJQK';
  const cardSuits = '♣♦♥♠';

  if (value === '10') return 9;

  const cardValue = value.length === 3 ? 'X' : value.slice(0, -1);
  const cardSuit = value.slice(-1);

  const valueIndex = cardValues.indexOf(cardValue);
  const suitIndex = cardSuits.indexOf(cardSuit);

  if (valueIndex === -1 || suitIndex === -1) {
    throw new Error(`Invalid card: '${value}'`);
  }

  return suitIndex * cardValues.length + valueIndex;
}
